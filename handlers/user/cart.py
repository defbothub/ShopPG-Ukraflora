...
async def process_cart(message: Message, state: FSMContext):
    ...
    if order is None:
        ordered_products = []
        await message.answer('Ваша корзина ще порожня.', reply_markup=user_markup)
    else:
...
async def clear_cart(message: Message):
    ...
    db.db_session.commit()
    await message.answer("Корзина очищена.", reply_markup=user_markup)
...
async def process_name(message: Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if 'address' in data.keys():
            ...
            await CheckoutState.next()
            await message.answer("🔹Доставка впродовж дня по Києву 100 ₴"
                                 "\n🔸У точно вказаний час по Києву 220 ₴"
                                 "\n🔹Доставка великогабаритних товарів від 400 ₴"
                                 "\n🔸При замовленні від 1500 ₴ доставка безкоштовна."
...
async def process_confirm(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.answer("Ваше замовлення прийняте ✅ "
                             ...
                             "\nОчікуйте на дзвінок нашого менеджера 🧑‍🔬", reply_markup=user_markup)
        ...
        await bot.send_message(chat_id=5791823682, text='\n'.join(text))

        await message.answer("Дякуємо за замовлення! "
                             "\nЩоб продовжити покупки, натисніть Menu", reply_markup=markup)
