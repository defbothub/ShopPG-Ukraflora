...
# Нажатие на "⚙ Налаштування винної карти"
...
async def add_category_callback_handler(query: CallbackQuery):
    await query.message.delete()
    await query.message.answer('Яка назва категорії?', reply_markup=admin_markup)
...
async def delete_category_handler(message: Message, state: FSMContext):
    async with state.proxy() as data:
        if 'category_index' in data.keys():
            ...
            db.db_session.commit()
            await message.answer('Готово!', reply_markup=admin_markup)
            await process_settings(message)
...
async def process_cancel(message: Message, state: FSMContext):
    await message.answer('Ок, відмінено!', reply_markup=admin_markup)
    await state.finish()
    await process_settings(message)
...
async def process_confirm_back(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.answer(f"{now_additive} Змінити ціну з <b>{data['additives'][now_additive]}</b>?",
